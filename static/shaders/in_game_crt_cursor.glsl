...
float sdTrail(vec2 p, vec2 currPos, vec2 currSize, vec2 prevPos, vec2 prevSize, float t) {
    ...
    vec2 corners[4] = vec2[4](currTopLeft, currTopRight, currBottomRight, currBottomLeft);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float tShape = 1.0 - pow(1.0 - clamp((iTime - iTimeCursorChange) / TIME_DURATION_FACTOR, 0.0, 1.0), 3.0);
    ...
    vec4 glowColor = mix(fragColor, mix(prevColor, currColor, dTip) + GLOW_COLOR_OFFSET_BRIGHTNESS, pow(dTip, 3.0));
