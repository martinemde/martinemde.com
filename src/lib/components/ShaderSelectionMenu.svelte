...
<button
  onclick={toggle}
  class="absolute top-2 left-2 rounded bg-surface-200-800 px-2 py-1 text-xs text-surface-950-50 opacity-50 hover:opacity-100"
...
{#if show}
  <div
    ...
    class="absolute top-12 left-2 max-h-[80%] w-64 overflow-auto rounded-lg border border-surface-300-700 bg-surface-50-950 p-3 shadow-lg"
  >
    ...
    <div class="space-y-1 pb-3">
      {#each shaders as shader, index (shader.name)}
        <label class="flex cursor-pointer items-center gap-2 rounded p-2 hover:bg-surface-100-900">
          <input
            type="checkbox"
            class="checkbox"
