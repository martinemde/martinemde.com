...
### Micropub files and the stream

The publisher writes Markdown files into `src/content/blog/`; it does not call a
blog API. The repository-root `publishing.json` lists the types the renderer
supports. The publisher may use that agreed list in its standard authenticated
`?q=config` response (`post-types`, the Supported Vocabulary extension):
https://indieweb.org/Micropub-extensions#Query_for_Supported_Vocabulary.
This file is a local integration contract, not a new discovery protocol.

Keep the publisher's `micropub: { type: [h-entry], properties: ... }` object intact.
Properties retain Micropub array values, including photo objects with `value` and
`alt`. `micropub.properties` determines title and entry kind; absent `name` means
untitled even if the compatibility `title` says "Untitled Post". Wrapper `date`,
`slug`, `published`, `description`, `author`, and `categories` remain supported.
Legacy articles without `micropub` retain their ordinary frontmatter.

The Markdown body is the authoritative published content, including any HTML,
photos, bookmark links, quotes, and commentary generated by the publisher. The
blog renders it once; it does not append another copy of `photo` or `bookmark-of`
from metadata. Original `micropub.properties.content` is editing source and aids
type discovery; it must not replace the published body. The publisher must keep
both synchronized when editing. Plain text vs HTML vs Markdown conversion is the
publisher's responsibility. Repository Markdown/HTML is trusted author content,
as it already is for MDsveX; do not fetch and insert arbitrary remote page HTML.
...
`/blog` and homepage Writing list articles. `/stream` includes all published
entries, grouped by Los Angeles calendar day; Recently shows non-article entries.
...
behavior with `noindex`; do not use a draft URL as an access-control mechanism.

Article expansion uses native details/summary, works without JS, and never closes
another entry automatically. j/k navigation is opt-in for the current page. It
leaves arrows, modified keys, editable fields, selected text, and interactive
controls alone. No custom feed role or roving tab order: links and disclosure
controls keep native keyboard and screen-reader behavior.

`src/lib/fixtures/publisher/` contains files generated by the separate publisher's
`parseMicropubRequest` + `generateMarkdownFile` serializer. Regenerate examples
there when changing the contract, then run `bun run test src/lib/components/Stream.test.ts`. These tests exercise the compiled Markdown
components, stream, feed HTML/XML, and share metadata together. Fixtures are not
published blog content. Add an integration fixture before advertising a new type.
...
PNG files; do not claim a live unfurl is updated without verifying deployment.
On Node 26, Vitest/jsdom can pick up Node's experimental `localStorage` instead
of the document's storage; run `NODE_OPTIONS=--no-experimental-webstorage bun run test`
when that causes existing tests to fail at `localStorage.clear()`.
