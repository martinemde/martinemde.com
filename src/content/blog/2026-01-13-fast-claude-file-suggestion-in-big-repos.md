...
Anthropic [anticipated][file-suggestion] this and provided `fileSuggestion` to allow us to improve performance on these big repositories.
...
Claude Code seems to filter suggestions on the main thread on every character.
...
This problem is maybe the worst part, since slow searches without a frozen UI would at least be tolerable.

I hacked a solution for this with claude, [file-suggestion.sh][latest], which builds a cache from
`git ls-files`, if available, falling back to `fd` or `find` which gets filtered with `ripgrep` and `fzf`.

Luckily, using a separate script also seems to background the filtering suggestions script.
This completely removes the UI lag on input everywhere, a surprise bonus that guarantees I will use some sort of suggestion script for the foreseeable future.

On our core repo, this drops the search time to about 62ms compared to around 1000ms without the index
The script also watches for any time that the .git/index or .git/HEAD is newer than the cache file, and automatically refreshes the cache.
...
To try it, make sure you have `ripgrep`, `fzf` and optionally `fd`.
